...
void ScenePBRShowcase::onCreate(SceneData& sceneData)
{
	//---------------------------------------------
	// Lighting
	directionalLight.SetLightDirection({ 0.0f, 1.0f, 0.5f });
	...
	blue_rubber.metallic = 1.0f;
	blue_rubber.roughness = 0.1f;
	...
	for (int i = 0; i < 5; i++)
	{
		...
		aluminum.metallic = metallic;

		sphere->model->SetPBRMaterial(aluminum);
